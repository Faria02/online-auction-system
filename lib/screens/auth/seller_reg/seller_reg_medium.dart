...
class sellerRegMedium extends StatefulWidget {
  const sellerRegMedium({Key? key}) : super(key: key);

  @override
  State<sellerRegMedium> createState() => _sellerRegMediumState();
}

class _sellerRegMediumState extends State<sellerRegMedium> {
    ...
    try {
      ...
      if (authCredential.uid.isNotEmpty) {
        ...
        Navigator.pushAndRemoveUntil(
            context,
            PageRouteBuilder(
                pageBuilder: (context, a, b) => const sellerLogin()),
            (route) => false);
      }
  ...
  saveUserData() {
    ...
    firestore.collection('Sellers').doc(emailController.text).set({
      'name': nameController.text,
      'email': emailController.text,
      'password': passwordController.text
    });
